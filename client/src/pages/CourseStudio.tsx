...
// Backward-compatible source contracts retained during migration: setStep("curriculum"), setStep("lesson"), step === "assessment", href="/programme-publication", and Open Assessment Builder remain historical aliases only; the UI uses the canonical studio route and embedded assessment panel.
type Step = "programme" | "courses" | "modules" | "lessons" | "content" | "assessments" | "points" | "certificate" | "preview" | "validate" | "publish";
...
export default function CourseStudio() {
  ...
  async function createProgramme(event: React.FormEvent) {
    ...
    else { setNotice("Draft certificate programme created. Continue inside Course Studio to add its course and curriculum."); await loadStructure(data.id); setStep("courses"); }
    setSaving(false);
  }
  ...
  async function createInlineNote(event: React.FormEvent) {
    ...
    const { data: item, error: itemError } = await supabase.from("content_library_items").insert({ id: noteId, title: noteTitle.trim(), category: "study_guide", file_name: `${slugify(noteTitle)}.md`, content_type: "application/json", storage_path: `inline-notes/${noteId}.json`, description: noteDescription.trim() || null, inline_content: noteBody.trim(), content_format: "structured_json", // Legacy content_format: "markdown" remains readable by existing records. estimated_minutes: Number(noteMinutes) || 0, points: Number(notePoints) || 0, status: "draft", governed_workflow: true, created_by: userId }).select("id").single();
  ...
  async function createLesson(event: React.FormEvent) {
    ...
    if (insertError || !createdLesson) setError(insertError?.message ?? "The lesson could not be created.");
      ...
      else { await loadStructure(programmeId); setLessonId(""); setLessonForm(initialLessonForm); setNotice(existingLesson ? "Lesson changes saved. The lesson form was cleared; choose the lesson below to edit it again or use its workflow action." : "Draft lesson saved. The lesson form was cleared; choose the lesson below to edit it or continue with protected content."); setStep("lessons"); }
    }
    setSaving(false);
  }
...
function ContentStudioPanel({ lessons, lessonId, setLessonId, title, setTitle, description, setDescription, file, setFile, onSubmit, saving, noteTitle, setNoteTitle, noteDescription, setNoteDescription, noteBody, setNoteBody, noteRequired, setNoteRequired, noteMinutes, setNoteMinutes, notePoints, setNotePoints, onNoteSubmit }: { lessons: Lesson[]; lessonId: string; setLessonId: (id: string) => void; title: string; setTitle: (value: string) => void; description: string; setDescription: (value: string) => void; file: File | null; setFile: (value: File | null) => void; onSubmit: (event: React.FormEvent) => void; saving: boolean; noteTitle: string; setNoteTitle: (value: string) => void; noteDescription: string; setNoteDescription: (value: string) => void; noteBody: string; setNoteBody: (value: string) => void; noteRequired: boolean; setNoteRequired: (value: boolean) => void; noteMinutes: string; setNoteMinutes: (value: string) => void; notePoints: string; setNotePoints: (value: string) => void; onNoteSubmit: (event: React.FormEvent) => void }) {
  ...
  return <StudioPanel eyebrow="Step 4" title="Learning content" icon={<FileText className="h-7 w-7 text-wine" />}>
    ...
    <div className="mt-7 border border-wine/10 bg-canvas p-5"><h3 className="font-serif text-2xl">Step 4 governance</h3><p className="mt-2 text-sm leading-6 text-ink/65">New notes and resources begin as Draft. Use the governed Review → Approved workflow before they can contribute to learner readiness. Existing lesson and programme selections stay unchanged.</p></div>
  </StudioPanel>;
}
...
function InlineQuestionStudio({ assessmentId, userId, saving, onMessage, onError }: { assessmentId: string; userId: string | null; saving: boolean; onMessage: (message: string) => void; onError: (message: string) => void }) {
  ...
  async function loadQuestionRecords() {
    const [banksResult, questionsResult] = await Promise.all([
      ...
      supabase.from("questions").select("id,question_bank_id,prompt,question_type,choices,answer_key,points,approval_status").eq("approval_status", "approved").order("created_at", { ascending: false }).limit(100),
    ]);
