...
export default function CourseStudio() {
  ...
  async function approveRecord(recordType: "course" | "module" | "lesson", recordId: string) {
    clearFeedback(); setSaving(true);
    const { error: approvalError } = await supabase.rpc("niu_approve_academic_record", { target_type: recordType, target_id: recordId });
    if (approvalError) setError(approvalError.message);
    else { setNotice(`${recordType[0].toUpperCase()}${recordType.slice(1)} approved. The programme record was not changed.`); await loadStructure(programmeId); await runReadiness(); }
    setSaving(false);
  }
...
function ApprovalAction({ recordType, recordId, status, onApprove, saving }: { recordType: "course" | "module" | "lesson"; recordId: string; status: string; onApprove: (recordType: "course" | "module" | "lesson", recordId: string) => Promise<void>; saving: boolean }) { const approved = status === "approved" || status === "published"; return <div className="my-5 flex flex-wrap items-center justify-between gap-3 border border-gold/40 bg-gold/10 p-4"><div><p className="text-sm font-bold">{recordType[0].toUpperCase() + recordType.slice(1)} status: {status}</p><p className="text-xs text-ink/60">Approval changes only this record and records an audit event.</p></div>{approved ? <span className="text-sm font-bold text-emerald-800">Approved</span> : <button type="button" disabled={saving} onClick={() => void onApprove(recordType, recordId)} className="button-secondary">Approve {recordType}</button>}</div>; }
