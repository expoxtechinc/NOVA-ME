...
type Readiness = { program_status: string; courses: number; approved_courses: number; modules: number; approved_modules: number; required_lessons: number; approved_required_lessons: number; required_lessons_with_material: number; governed_assessments: number; ready_governed_assessments: number; ready: boolean };
...
const steps: { id: Step; label: string; detail: string }[] = [
  ...
  { id: "curriculum", label: "Course structure", detail: "Courses and ordered modules" },
  { id: "lesson", label: "Lessons", detail: "Learning activity details" },
  ...
  { id: "rules", label: "Completion rules", detail: "Grading and eligibility" },
  { id: "certificate", label: "Certificate", detail: "Template and verification" },
  { id: "preview", label: "Full programme review", detail: "Read-only learner view" },
  { id: "review", label: "Validation & publication", detail: "Quality gate and workflow" },
];
const levels = ["foundation", "developing", "applied", "advanced", "capstone"];
const lessonKinds = ["article", "reading", "video", "flashcards", "quiz", "assignment", "assessment"];
...
export default function CourseStudio() {
  ...
  const checks = useMemo(() => [
    ...
    { label: "Approved required courses in this programme", ok: Boolean(readiness?.courses && readiness.approved_courses === readiness.courses) },
    { label: "Approved modules in this programme", ok: Boolean(readiness?.modules && readiness.approved_modules === readiness.modules) },
    { label: "Approved required lessons with learning structure", ok: Boolean(readiness?.required_lessons && readiness.approved_required_lessons === readiness.required_lessons && requiredLessons.every(item => item.title && item.estimated_minutes >= 0)) },
    ...
    { label: "Assessment requirements satisfied", ok: Boolean(readiness && readiness.governed_assessments === readiness.ready_governed_assessments) },
  ...
  async function loadStructure(nextProgrammeId?: string) {
    ...
    if (!activeProgrammeId) { setCourses([]); setModules([]); setLessons([]); return; }
    ...
    if (!ids.length) { setModules([]); setLessons([]); return; }
  ...
  if (!staff) return <SiteShell><section className="mx-auto flex min-h-[55vh] max-w-2xl flex-col items-center justify-center px-5 text-center"><ShieldAlert className="h-10 w-10 text-wine" /><h1 className="mt-5 font-serif text-4xl">Course Studio is restricted.</h1><p className="mt-3 leading-7 text-ink/65">Only authorised instructional and administrative roles can create or revise NIU academic content.</p><Link href={role ? "/portal" : "/signin"} className="button-primary mt-7">{role ? "Return to My NIU" : "Sign in to NIU"}</Link></section></SiteShell>;
  ...
  const canEnterStep = (id: Step) => id === "programme" || (id === "curriculum" && Boolean(selectedProgramme)) || (id === "lesson" && courses.length > 0 && modules.length > 0) || (id === "content" && lessons.length > 0) || (id === "assessment" && courses.length > 0) || (id === "rules" && Boolean(selectedProgramme)) || (id === "certificate" && Boolean(selectedProgramme)) || (id === "preview" && lessons.length > 0) || (id === "review" && Boolean(selectedProgramme));
  ...
  return <SiteShell>
    <section className="border-b border-wine/10 bg-ink text-paper"><div className="mx-auto max-w-[1500px] px-5 py-10 sm:px-8"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="eyebrow text-gold">Unified academic authoring</p><h1 className="mt-3 font-serif text-5xl tracking-[-0.04em]">Programme Builder</h1><p className="mt-3 max-w-3xl leading-7 text-paper/70">Create a complete certificate programme in one governed workspace. The tree, editors, preview, quality gate, and publication hand-off stay together while existing authorization and audit controls remain authoritative.</p></div><div className="flex flex-wrap gap-3 text-sm"><Link href="/admin" className="border border-paper/30 px-4 py-3 font-bold text-paper">Admin dashboard</Link><Link href="/programme-publication" className="border border-gold px-4 py-3 font-bold text-gold">Publication gate</Link></div></div></div></section>
    <section className="mx-auto max-w-[1500px] px-4 py-6 sm:px-8"><div className="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)_300px]">
      <aside className="border border-wine/10 bg-white p-4 lg:sticky lg:top-5 lg:h-fit"><div className="flex items-center gap-2 border-b border-wine/10 pb-4"><Sparkles className="h-5 w-5 text-wine" /><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-wine">Studio steps</p><p className="text-sm text-ink/60">One workspace, no dead ends</p></div></div><nav className="mt-4 grid gap-1">{steps.map((item, index) => <button key={item.id} type="button" disabled={!canEnterStep(item.id)} onClick={() => { if (canEnterStep(item.id)) { setStep(item.id); clearFeedback(); } }} className={`flex items-start gap-3 p-3 text-left transition ${step === item.id ? "bg-wine text-paper" : canEnterStep(item.id) ? "text-ink hover:bg-canvas" : "cursor-not-allowed text-ink/35"}`}><span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-xs font-bold ${step === item.id ? "bg-gold text-ink" : "bg-canvas text-wine"}`}>{index + 1}</span><span><span className="block text-sm font-bold">{item.label}</span><span className={`mt-0.5 block text-xs ${step === item.id ? "text-paper/70" : "text-ink/55"}`}>{item.detail}</span></span></button>)}</nav></aside>
