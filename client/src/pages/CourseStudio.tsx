import { AlertCircle, ArrowRight, BookOpen, Check, ChevronRight, ClipboardCheck, FileText, Layers3, LoaderCircle, Plus, Save, Send, ShieldAlert, Sparkles, Target, Video, X } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { Link } from "wouter";
import SiteShell from "@/components/SiteShell";
import { supabase, supabaseConfigured } from "@/lib/supabase";
import { LESSON_KIND_OPTIONS } from "@shared/lessonKinds";
import StructuredLessonEditor, { getStructuredText } from "@/components/StructuredLessonEditor";
import { trpc } from "@/lib/trpc";

type Role = "student" | "instructor" | "administrator" | "super_admin";
// The canonical studio owns programme, curriculum, content, assessment, review, preview, validation, publication, and release-evidence workflow; older entry points are compatibility aliases in App.tsx.
type Step = "programme" | "courses" | "modules" | "lessons" | "content" | "assessments" | "points" | "certificate" | "preview" | "transcript" | "validate" | "publish" | "release";
...
type ContentItem = { id: string; title: string; category: string; file_name: string; content_type: string; status: string | null; content_format?: string | null; inline_content?: string | null; storage_path?: string | null; lesson_id: string };
...
const steps: { id: Step; label: string; detail: string }[] = [
  ...
  { id: "courses", label: "Courses", detail: "Create and order courses" },
  { id: "modules", label: "Modules", detail: "Build course structure" },
  { id: "lessons", label: "Lessons", detail: "Objectives, activities, support" },
  { id: "content", label: "Content & resources", detail: "Notes, media, resources" },
  { id: "assessments", label: "Assessments", detail: "Checks, questions, exams" },
  { id: "points", label: "Points", detail: "Grading and completion" },
  { id: "certificate", label: "Certificate", detail: "Template and verification" },
  { id: "preview", label: "Preview", detail: "Read-only learner view" },
  { id: "transcript", label: "Transcript", detail: "Completion and credential view" },
  { id: "validate", label: "Validate", detail: "Quality and relationship checks" },
  { id: "publish", label: "Publish", detail: "Authorized release gate" },
  { id: "release", label: "Release evidence", detail: "Final audit confirmation" },
];
const levels = ["foundation", "developing", "applied", "advanced", "capstone"];
const lessonKinds = LESSON_KIND_OPTIONS;
const learningResourceMimeTypes: Record<string, string> = {
  ".pdf": "application/pdf",
...
function resourceContentType(file: File) {
  const extension = `.${file.name.split(".").pop()?.toLowerCase() ?? ""}`;
  return learningResourceMimeTypes[extension] ?? file.type.toLowerCase();
}
function isSupportedLearningResource(file: File) {
  return Boolean(resourceContentType(file) && Object.values(learningResourceMimeTypes).includes(resourceContentType(file)));
}
...
export default function CourseStudio() {
  ...
  const [noteMinutes, setNoteMinutes] = useState("10");
  ...
  const [aiBlueprint, setAiBlueprint] = useState<{ programme?: { title?: string; description?: string }; courses?: Array<{ title: string; modules?: Array<{ title: string; lessons?: Array<{ title: string }> }> }> } | null>(null);
  ...
  const unlockedStep = !programmeId ? 0 : !courseId ? 1 : !moduleId ? 2 : !selectedLessons.length ? 3 : contentCount === 0 ? 4 : assessmentCount === 0 ? 5 : 12;
  const checks = useMemo(() => [
    ...
    { label: "All required courses approved and connected", ok: Boolean(readiness && readiness.courses > 0 && readiness.courses === readiness.approved_courses) },
    { label: "All required modules approved and connected", ok: Boolean(readiness && readiness.modules > 0 && readiness.modules === readiness.approved_modules) },
    { label: "All required lessons approved with learning structure", ok: Boolean(readiness && readiness.required_lessons > 0 && readiness.required_lessons === readiness.approved_required_lessons && requiredLessons.every(item => item.title && item.estimated_minutes >= 0)) },
    { label: "Protected material attached to every required lesson", ok: Boolean(readiness && readiness.required_lessons > 0 && readiness.required_lessons_with_material === readiness.required_lessons) },
    { label: "All governed assessments are ready", ok: Boolean(readiness && readiness.governed_assessments > 0 && readiness.governed_assessments === readiness.ready_governed_assessments) },
    { label: "Certificate template and verification review", ok: Boolean(readiness?.certificate_templates === 1 && readiness.approved_certificate_templates === 1) },
  ], [readiness, requiredLessons, selectedProgramme]);

  const clearFeedback = () => { setNotice(null); setError(null); setReadiness(null); };
  const aiPlanMutation = trpc.aiBuilder.createPlan.useMutation({
    onSuccess(result) { setAiBlueprint(result.blueprint as typeof aiBlueprint); setNotice("AI planning produced a reviewable blueprint only. No academic records, questions, materials, or publication were created."); setError(null); },
    onError(result) { setError(result.message); setNotice(null); },
  });
  const generateAiBlueprint = () => {
    const topic = aiTopic.trim() || selectedProgramme?.name;
    if (!topic) { setError("Enter an academic topic or select a programme before requesting an AI blueprint."); return; }
    aiPlanMutation.mutate({ topic, settings: { programmeCode: selectedProgramme?.code || undefined, difficulty: "intermediate", learningHours: Math.max(1, Number(selectedProgramme?.duration_hours ?? 36)), academicDepth: "applied", numberOfCourses: 1, estimatedModulesPerCourse: 2, estimatedLessonsPerModule: 3, generationDepth: "standard", researchDepth: "standard", visualGeneration: false, assessmentGeneration: true, minimumPassingScore: selectedProgramme?.required_score || 70, certificateTemplate: selectedProgramme?.certificate_template_key || undefined } });
  };
  ...
  useEffect(() => {
    const current = programmes.find(item => item.id === programmeId);
    if (!current) return;
    const completion = current.completion_requirements && typeof current.completion_requirements === "object" ? current.completion_requirements : {};
    const entryRequirements = Array.isArray(completion.entry_requirements) ? completion.entry_requirements.join("\\n") : "";
    const completionRules = Array.isArray(completion.completion_rules) ? completion.completion_rules.join("\\n") : "";
    setProgrammeForm(form => ({ ...form, name: current.name, code: current.code, departmentId: current.department_id, description: current.description, hours: String(current.duration_hours ?? 0), difficulty: current.difficulty, score: String(current.required_score ?? 70), template: current.certificate_template_key ?? form.template, image: current.image_path ?? "", requirements: entryRequirements, completion: completionRules }));
    setCertificateForm(form => ({ ...form, templateKey: current.certificate_template_key ?? form.templateKey }));
  }, [programmeId, programmes]);

  async function transitionRecord(recordType: "programme" | "course" | "module" | "lesson" | "content_item" | "assessment" | "certificate_template", recordId: string, targetStatus: "review" | "approved") {
    ...
    else { setNotice(`${recordType.replace("_", " ")} moved to ${targetStatus}. ${recordType === "programme" ? "The programme status was changed." : "The programme record was not changed."}`); await loadStructure(programmeId); await runReadiness(); }
    setSaving(false);
  }
  ...
  async function runReadiness() {
    ...
    const { data, error: rpcError } = await supabase.rpc("niu_get_programme_readiness", { programme_id: programmeId });
  ...
  async function publishProgramme() {
    if (!canPublish) { setError("Only an administrator or Super Administrator may publish a programme bundle."); return; }
    if (!programmeId || !readiness?.ready) { setError("Run Validate and resolve every readiness requirement before publishing."); return; }
    if (!window.confirm("Publish this completed NIU certificate programme bundle? The database gate will change only the verified package records and record an audit event.")) return;
    setSaving(true); clearFeedback();
    const { data, error: publishError } = await supabase.rpc("niu_publish_programme_bundle", { target_program_id: programmeId });
    if (publishError) setError(publishError.message);
    else { setReadiness(data as Readiness); setNotice("The complete certificate programme bundle was published and an NIU audit event was recorded."); await loadStructure(programmeId); }
    setSaving(false);
  }
  ...
  async function loadStructure(nextProgrammeId?: string) {
    ...
    if (lessonIds.length) {
      const { data: contentData, error: contentError } = await supabase.from("lesson_content_items").select("lesson_id,content_item_id,content_library_items(id,title,category,file_name,content_type,status,content_format,inline_content,storage_path)").in("lesson_id", lessonIds).order("position");
      if (contentError) throw new Error("NIU could not load the protected learning-content attachments.");
      const nextContentItems = (contentData ?? []).map(item => {
        const resource = Array.isArray(item.content_library_items) ? item.content_library_items[0] : item.content_library_items;
        return resource ? { ...resource, lesson_id: item.lesson_id } : null;
      }).filter(Boolean) as ContentItem[];
      setContentItems(nextContentItems);
      setContentCount(nextContentItems.length);
    } else { setContentItems([]); setContentCount(0); }
  ...
  async function createProgramme(event: React.FormEvent) {
    ...
    const continueWorkflow = Boolean((event.nativeEvent as SubmitEvent).submitter && ((event.nativeEvent as SubmitEvent).submitter as HTMLElement).dataset.continue === "true");
    const current = selectedProgramme;
    if (current && ["approved", "published"].includes(current.status)) { setError("Approved or published programmes are locked. Create a new governed version before changing programme information."); return; }
    setSaving(true);
    const payload = { department_id: programmeForm.departmentId, name: programmeForm.name.trim(), code: programmeForm.code.trim().toUpperCase(), description: programmeForm.description.trim(), objectives: listItems(programmeForm.objectives), learning_outcomes: listItems(programmeForm.outcomes), duration_hours: Number(programmeForm.hours) || 0, difficulty: programmeForm.difficulty, required_score: Number(programmeForm.score) || 70, completion_requirements: { entry_requirements: listItems(programmeForm.requirements), completion_rules: listItems(programmeForm.completion) }, certificate_template_key: programmeForm.template.trim() || null, image_path: programmeForm.image.trim() || null, award_type: "certificate", governed_workflow: true };
    const result = current ? await supabase.from("certificate_programs").update({ ...payload, updated_at: new Date().toISOString() }).eq("id", current.id).select("id").single() : await supabase.from("certificate_programs").insert({ ...payload, status: "draft", created_by: userId }).select("id").single();
    if (result.error || !result.data) setError(result.error?.message ?? "The draft certificate programme could not be saved.");
    else { setNotice(current ? "Draft programme information saved. Existing course and curriculum records were preserved." : "Draft certificate programme created. Continue inside Course Studio to add its course and curriculum."); await loadStructure(result.data.id); if (continueWorkflow) setStep("courses"); }
    setSaving(false);
  }
  ...
  async function createCourse(event: React.FormEvent) {
    ...
    else { setNotice("Draft course created and connected. Add modules without leaving the Academic Production Studio."); await loadStructure(programmeId); setCourseId(created.id); setModuleId(""); setCourseForm({ title: "", description: "", category: "Professional development", level: "beginner", minutes: "60", outcomes: "", requirements: "" }); setStep("modules"); }
    setSaving(false);
  }
  ...
  async function createModule(event: React.FormEvent) {
    ...
    if (insertError || !created) setError(insertError?.message ?? "The module could not be created.");
      ...
      else { setNotice("Draft module saved and connected to this programme package. Add its first lesson below."); await loadStructure(programmeId); setModuleId(created.id); setStep("lessons"); }
    }
    setSaving(false);
  }
  ...
  async function uploadResource(event: React.FormEvent) {
    ...
    if (!isSupportedLearningResource(resourceFile)) { setError("Choose a supported PDF, Word, PowerPoint, text, Markdown, image, audio, or video resource."); return; }
    ...
    const contentType = resourceContentType(resourceFile);
    const { error: uploadError } = await supabase.storage.from("niu-learning-materials").upload(storagePath, resourceFile, { contentType, upsert: false });
    if (uploadError) { setError(uploadError.message); setSaving(false); return; }
    const { data: item, error: itemError } = await supabase.from("content_library_items").insert({ title: resourceTitle.trim(), category: contentType.startsWith("image/") ? "image" : contentType.startsWith("audio/") ? "audio" : contentType.startsWith("video/") ? "video" : contentType.includes("presentation") ? "presentation" : "document", file_name: resourceFile.name, content_type: contentType, storage_path: storagePath, description: resourceDescription.trim() || null, status: "draft", governed_workflow: true, created_by: userId }).select("id").single();
    ...
    else { setNotice("Private learning resource uploaded and attached to the selected lesson. Learners will receive it only through enrolled-course access."); setResourceTitle(""); setResourceDescription(""); setResourceFile(null); await loadStructure(programmeId); }
    setSaving(false);
  }

  async function createInlineNote(event: React.FormEvent) {
    event.preventDefault(); clearFeedback();
    const targetLessonId = lessonId || selectedLessons[0]?.id;
    if (!userId || !targetLessonId || noteTitle.trim().length < 3 || getStructuredText(noteBody).trim().length < 40) { setError("Choose a lesson, provide a note title, and write at least 40 characters of meaningful learning content."); return; }
    const targetLesson = lessons.find(item => item.id === targetLessonId);
    if (targetLesson?.status === "approved" || targetLesson?.status === "published") { setError("Approved lessons are locked. Create a revision before changing their learning content."); return; }
    setSaving(true);
    const { data: attachedRows, error: attachedError } = await supabase.from("lesson_content_items").select("content_item_id").eq("lesson_id", targetLessonId);
    if (attachedError) { setError(`NIU could not inspect existing lesson content: ${attachedError.message}`); setSaving(false); return; }
    const attachedIds = (attachedRows ?? []).map(item => item.content_item_id);
    if (attachedIds.length) {
      const { data: existingNotes, error: duplicateError } = await supabase.from("content_library_items").select("id,title").in("id", attachedIds);
      if (duplicateError) { setError(`NIU could not check for duplicate lesson notes: ${duplicateError.message}`); setSaving(false); return; }
      const duplicate = (existingNotes ?? []).find(item => item.title.trim().toLocaleLowerCase() === noteTitle.trim().toLocaleLowerCase());
      if (duplicate) { setError(`A learning item titled “${duplicate.title}” is already attached to this lesson. Edit or reuse it instead of creating a duplicate.`); setSaving(false); return; }
    }
    const noteId = crypto.randomUUID();
    // Legacy content_format: "markdown" remains readable by existing records.
    const { data: item, error: itemError } = await supabase.from("content_library_items").insert({ id: noteId, title: noteTitle.trim(), category: "study_guide", file_name: `${slugify(noteTitle)}.md`, content_type: "application/json", storage_path: `inline-notes/${noteId}.json`, description: noteDescription.trim() || null, inline_content: noteBody.trim(), content_format: "structured_json", estimated_minutes: Number(noteMinutes) || 0, points: Number(notePoints) || 0, status: "draft", governed_workflow: true, created_by: userId }).select("id").single();
    if (itemError || !item) { setError(`Learning note could not be saved: ${itemError?.message ?? "database returned no record"}`); setSaving(false); return; }
    const { data: positions, error: positionError } = await supabase.from("lesson_content_items").select("position").eq("lesson_id", targetLessonId).order("position", { ascending: false }).limit(1);
    if (positionError) { await supabase.from("content_library_items").delete().eq("id", item.id); setError(`Learning note was saved but its lesson position could not be loaded: ${positionError.message}`); setSaving(false); return; }
    const { error: attachmentError } = await supabase.from("lesson_content_items").insert({ lesson_id: targetLessonId, content_item_id: item.id, position: Number(positions?.[0]?.position ?? -1) + 1, is_required: noteRequired });
    if (attachmentError) { await supabase.from("content_library_items").delete().eq("id", item.id); setError(`Learning note could not be attached to the selected lesson: ${attachmentError.message}`); setSaving(false); return; }
    setNoteTitle(""); setNoteDescription(""); setNoteBody(""); setNoteRequired(true); setNoteMinutes("10"); setNotePoints("0");
    setNotice("Draft learning notes saved and attached to the selected lesson. Submit the content through the governed review workflow before publication.");
    await loadStructure(programmeId); await runReadiness(); setSaving(false);
  }
  ...
  useEffect(() => {
    ...
    if (!current || step !== "lessons") return;
  ...
  async function createLesson(event: React.FormEvent) {
    ...
    if (insertError || !createdLesson) setError(insertError?.message ?? "The lesson could not be created.");
      ...
      else { await loadStructure(programmeId); setLessonId(""); setLessonForm(initialLessonForm); setNotice(existingLesson ? "Lesson changes saved. The lesson form was cleared; choose the lesson below to edit it again or use its workflow action." : "Draft lesson saved. The lesson form was cleared; continue with protected content."); setStep(existingLesson ? "lessons" : "content"); }
    }
    setSaving(false);
  }
  ...
  return <SiteShell>
    <section className="border-b border-wine/10 bg-ink text-paper"><div className="mx-auto max-w-[1500px] px-5 py-10 sm:px-8"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="eyebrow text-gold">Unified academic authoring</p><h1 className="mt-3 font-serif text-5xl tracking-[-0.04em]">Programme Builder</h1><p className="mt-3 max-w-3xl leading-7 text-paper/70">Create a complete certificate programme in one governed workspace. Steps unlock in order, completed sections remain editable, and every save stays draft-only until the authorised publication gate approves the selected package.</p></div><div className="flex flex-wrap gap-3 text-sm"><Link href="/admin" className="border border-paper/30 px-4 py-3 font-bold text-paper">Admin dashboard</Link><button type="button" onClick={() => setStep("publish")} className="border border-gold px-4 py-3 font-bold text-gold">Publish step</button></div></div></div></section>
    <section className="mx-auto max-w-[1500px] px-4 py-6 sm:px-8"><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
      <aside className="border border-wine/10 bg-white p-4 lg:col-span-2"><div className="flex items-center gap-2 border-b border-wine/10 pb-4"><Sparkles className="h-5 w-5 text-wine" /><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-wine">Academic Production Studio</p><p className="text-sm text-ink/60">One governed workspace, no dead ends</p></div></div><nav className="mt-4 flex min-w-max gap-2 overflow-x-auto pb-1" aria-label="Academic production workflow">{steps.map((item, index) => <button key={item.id} type="button" disabled={index > unlockedStep} onClick={() => { if (index <= unlockedStep) { setStep(item.id); clearFeedback(); } }} className={`flex min-w-[128px] items-center gap-2 border px-3 py-2 text-left transition ${step === item.id ? "border-wine bg-wine text-paper" : index > unlockedStep ? "cursor-not-allowed border-wine/10 text-ink/30" : "border-wine/15 text-ink hover:bg-canvas"}`}><span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-xs font-bold ${step === item.id ? "bg-gold text-ink" : "bg-canvas text-wine"}`}>{index + 1}</span><span><span className="block text-xs font-bold">{item.label}</span><span className={`mt-0.5 block text-[10px] ${step === item.id ? "text-paper/70" : "text-ink/55"}`}>{item.detail}</span></span></button>)}</nav></aside>
      <main className="min-w-0">
        ...
        <div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">{steps.find(item => item.id === step)?.label}</p><h2 className="mt-2 font-serif text-4xl">{selectedProgramme?.name ?? "Start a new certificate programme"}</h2><p className="mt-2 text-sm text-ink/60">{selectedProgramme ? `${selectedProgramme.code} · ${selectedProgramme.status} · certificate-only` : "Draft records stay unpublished until authorised review."}</p><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-ink/55"><span>Programme: <strong className="text-ink/75">{selectedProgramme?.name ?? "Not selected"}</strong></span><span>Course: <strong className="text-ink/75">{selectedCourse?.title ?? "Not selected"}</strong></span><span>Module: <strong className="text-ink/75">{selectedModule?.title ?? "Not selected"}</strong></span><span>Lesson: <strong className="text-ink/75">{lessons.find(item => item.id === lessonId)?.title ?? "Not selected"}</strong></span></div><p className="mt-2 text-xs text-ink/50">{autosavedAt ? `Private browser draft saved ${new Date(autosavedAt).toLocaleTimeString()}. Database saves remain governed and draft-only.` : "Private browser draft autosave is ready."}</p></div><div className="flex flex-wrap gap-2"><label className="grid gap-1 text-xs font-bold uppercase tracking-[0.12em] text-ink/55">Programme<select value={programmeId} onChange={event => void loadStructure(event.target.value)} className="min-w-[220px] border border-wine/20 bg-white px-3 py-2 text-sm font-normal normal-case tracking-normal"><option value="">Choose a programme</option>{programmes.map(item => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label><label className="grid gap-1 text-xs font-bold uppercase tracking-[0.12em] text-ink/55">Course<select value={courseId} onChange={event => { setCourseId(event.target.value); setModuleId(modules.find(item => item.course_id === event.target.value)?.id ?? ""); }} className="min-w-[220px] border border-wine/20 bg-white px-3 py-2 text-sm font-normal normal-case tracking-normal"><option value="">Choose a course</option>{courses.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></div></div>

        {step === "programme" && <section className="border border-wine/10 bg-white p-6"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Step 1</p><h3 className="mt-2 font-serif text-3xl">Programme information</h3><p className="mt-2 text-sm leading-6 text-ink/65">A programme is always certificate-only. Save a draft first; this does not publish a public listing.</p></div><Target className="h-7 w-7 text-wine" /></div>{selectedProgramme && <WorkflowAction recordType="programme" recordId={selectedProgramme.id} status={selectedProgramme.status} onTransition={transitionRecord} saving={saving} />}<form onSubmit={createProgramme} className="mt-7 grid gap-4 md:grid-cols-2">{programmeFields.map(([label, key, type]) => <label key={key} className="grid gap-2 text-sm font-semibold">{label}<input type={type} min={type === "number" ? "0" : undefined} value={programmeForm[key]} onChange={event => update(setProgrammeForm, key, event.target.value)} className="border border-wine/20 px-3 py-3 font-normal" /></label>)}<label className="grid gap-2 text-sm font-semibold">Department<select value={programmeForm.departmentId} onChange={event => update(setProgrammeForm, "departmentId", event.target.value)} className="border border-wine/20 bg-white px-3 py-3 font-normal"><option value="">Choose a department</option>{departments.map(item => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label><label className="grid gap-2 text-sm font-semibold">Difficulty<select value={programmeForm.difficulty} onChange={event => update(setProgrammeForm, "difficulty", event.target.value)} className="border border-wine/20 bg-white px-3 py-3 font-normal"><option value="beginner">Beginner</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option></select></label><Field label="Description" value={programmeForm.description} onChange={value => update(setProgrammeForm, "description", value)} wide /><Field label="Programme objectives" value={programmeForm.objectives} onChange={value => update(setProgrammeForm, "objectives", value)} /><Field label="Learning outcomes" value={programmeForm.outcomes} onChange={value => update(setProgrammeForm, "outcomes", value)} /><Field label="Entry requirements" value={programmeForm.requirements} onChange={value => update(setProgrammeForm, "requirements", value)} /><Field label="Completion requirements" value={programmeForm.completion} onChange={value => update(setProgrammeForm, "completion", value)} /><Field label="Certificate template key" value={programmeForm.template} onChange={value => update(setProgrammeForm, "template", value)} /><Field label="Programme image or approved reference" value={programmeForm.image} onChange={value => update(setProgrammeForm, "image", value)} /><div className="md:col-span-2 flex flex-wrap gap-3"><button type="submit" onClick={event => { (event.currentTarget as HTMLButtonElement).dataset.continue = "false"; }} disabled={saving} className="button-primary"><Save className="h-4 w-4" />{saving ? "Saving draft…" : "Save draft programme"}</button><button type="submit" onClick={event => { (event.currentTarget as HTMLButtonElement).dataset.continue = "true"; }} disabled={saving} className="button-secondary">Save & continue <ArrowRight className="h-4 w-4" /></button></div></form></section>}
        {step === "programme" && <StudioPanel eyebrow="AI-assisted drafting" title="Generate a governed blueprint" icon={<Sparkles className="h-7 w-7 text-wine" />}><p className="leading-7 text-ink/65">Use the existing server-side Gemini workflow to plan a certificate programme. The result is a reviewable blueprint; no courses, lessons, questions, media, approval, or publication are created until an authorized person explicitly continues.</p><div className="mt-5 flex flex-col gap-3 sm:flex-row"><input value={aiTopic} onChange={event => setAiTopic(event.target.value)} placeholder="Academic topic or programme name" className="min-w-0 flex-1 border border-wine/20 px-3 py-3" /><button type="button" onClick={generateAiBlueprint} disabled={aiPlanMutation.isPending} className="button-secondary">{aiPlanMutation.isPending ? "Planning securely…" : "Generate draft blueprint"}</button></div>{aiBlueprint && <div className="mt-6 border border-wine/10 bg-canvas p-5"><p className="eyebrow">Reviewable AI output</p><h4 className="mt-2 font-serif text-2xl">{aiBlueprint.programme?.title ?? "Untitled blueprint"}</h4><p className="mt-2 text-sm leading-6 text-ink/65">{aiBlueprint.programme?.description ?? "No description returned."}</p><p className="mt-4 text-sm font-semibold text-wine">{aiBlueprint.courses?.length ?? 0} course draft outline{aiBlueprint.courses?.length === 1 ? "" : "s"} · evidence and staff review required before generation</p><div className="mt-4 grid gap-2">{aiBlueprint.courses?.map(course => <div key={course.title} className="border border-wine/10 bg-white p-3 text-sm"><strong>{course.title}</strong><span className="ml-2 text-ink/55">{course.modules?.length ?? 0} modules</span></div>)}</div></div>}</StudioPanel>}

        {(step === "courses" || step === "modules") && <div className="grid gap-6"><section className="border border-wine/10 bg-white p-6"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">{step === "modules" ? "Step 3" : "Step 2"}</p><h3 className="mt-2 font-serif text-3xl">{step === "modules" ? "Modules in this course" : "Courses in this programme"}</h3><p className="mt-2 text-sm leading-6 text-ink/65">Create and order real Supabase records inside the current package. Every course and module is linked automatically; no administrator-entered IDs are required.</p></div><Layers3 className="h-7 w-7 text-wine" /></div>{selectedProgramme && <div className="mt-6 border-l-4 border-gold bg-gold/10 p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-wine">Selected programme</p><p className="mt-1 font-serif text-2xl">{selectedProgramme.code} · {selectedProgramme.name}</p><p className="mt-1 text-sm text-ink/60">Programme information is read-only in Step 2. Saving below creates a separate course record.</p></div>}<CurriculumTree courses={courses} modules={modules} lessons={lessons} courseId={courseId} moduleId={moduleId} onCourse={setCourseId} onModule={id => { setModuleId(id); setStep("lessons"); }} onLesson={id => { const lesson = lessons.find(item => item.id === id); if (lesson) selectLessonForEdit(lesson); setStep("lessons"); }} />{selectedCourse && <WorkflowAction recordType="course" recordId={selectedCourse.id} status={selectedCourse.status} onTransition={transitionRecord} saving={saving} />}{selectedModule && <WorkflowAction recordType="module" recordId={selectedModule.id} status={selectedModule.status} onTransition={transitionRecord} saving={saving} />}<CourseForm form={courseForm} setForm={setCourseForm} onSubmit={createCourse} saving={saving} disabled={!selectedProgramme} /><div className="mt-6 border-t border-wine/10 pt-6"><ModuleForm form={moduleForm} setForm={setModuleForm} onSubmit={createModule} saving={saving} disabled={!courseId} /></div></section></div>}

        {step === "lessons" && <section className="border border-wine/10 bg-white p-6"><p className="eyebrow">Step 4</p><h3 className="mt-2 font-serif text-3xl">Lesson builder</h3><p className="mt-2 text-sm leading-6 text-ink/65">Lessons stay inside their selected module. Add objectives, duration, points, transcript, and captions before connecting protected resources.</p><div className="mt-5 flex flex-wrap gap-2">{modules.filter(item => item.course_id === courseId).map(item => <button type="button" key={item.id} onClick={() => setModuleId(item.id)} className={`border px-3 py-2 text-sm font-bold ${item.id === moduleId ? "border-wine bg-wine text-paper" : "border-wine/20 text-wine"}`}>Module {item.position + 1}: {item.title}</button>)}</div>{selectedModule && <div className="mt-6 border-l-4 border-gold bg-gold/10 p-4 text-sm"><b>{selectedModule.title}</b><span className="ml-2 text-ink/60">{selectedModule.learning_level} · {selectedModule.estimated_minutes} minutes</span></div>}{selectedLessons.find(item => item.id === lessonId) && <WorkflowAction recordType="lesson" recordId={lessonId} status={selectedLessons.find(item => item.id === lessonId)?.status ?? "draft"} onTransition={transitionRecord} saving={saving} />}<LessonForm form={lessonForm} setForm={setLessonForm} onSubmit={createLesson} saving={saving} disabled={!moduleId} /><div className="mt-8 border-t border-wine/10 pt-6"><h4 className="font-serif text-2xl">Existing lessons in this module</h4>{selectedLessons.length ? <div className="mt-4 grid gap-3">{selectedLessons.map(lesson => <article key={lesson.id} className="border border-wine/10 bg-canvas p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold">{lesson.position + 1}. {lesson.title}</p><p className="text-xs uppercase tracking-[0.12em] text-ink/50">Status: {lesson.status} · {lesson.is_required ? "Required" : "Optional"}</p></div><button type="button" onClick={() => selectLessonForEdit(lesson)} disabled={lesson.status === "approved" || lesson.status === "published"} className="button-secondary">{lesson.status === "approved" || lesson.status === "published" ? "Locked" : "Edit lesson"}</button></div><WorkflowAction recordType="lesson" recordId={lesson.id} status={lesson.status} onTransition={transitionRecord} saving={saving} /></article>)}</div> : <p className="mt-3 text-sm text-ink/60">No lessons yet. Save the first lesson above.</p>}</div></section>}

        {step === "content" && <ContentStudioPanel lessons={selectedLessons} items={contentItems} lessonId={lessonId || selectedLessons[0]?.id || ""} setLessonId={setLessonId} title={resourceTitle} setTitle={setResourceTitle} description={resourceDescription} setDescription={setResourceDescription} file={resourceFile} setFile={setResourceFile} onSubmit={uploadResource} saving={saving} noteTitle={noteTitle} setNoteTitle={setNoteTitle} noteDescription={noteDescription} setNoteDescription={setNoteDescription} noteBody={noteBody} setNoteBody={setNoteBody} noteRequired={noteRequired} setNoteRequired={setNoteRequired} noteMinutes={noteMinutes} setNoteMinutes={setNoteMinutes} notePoints={notePoints} setNotePoints={setNotePoints} onNoteSubmit={createInlineNote} onTransition={transitionRecord} />}

        {step === "assessments" && <AssessmentStudioPanel form={assessmentForm} setForm={setAssessmentForm} onSubmit={createAssessment} saving={saving} disabled={!courseId} assessment={assessments.find(item => item.id === assessmentId) ?? null} rulesForm={rulesForm} setRulesForm={setRulesForm} onSaveRules={saveAssessmentRules} onValidate={runReadiness} readiness={readiness} onTransition={(_recordType, recordId, targetStatus) => transitionRecord("assessment", recordId, targetStatus)} userId={userId} onMessage={message => setNotice(message)} onError={message => setError(message)} />}

        {step === "points" && <StudioPanel eyebrow="Step 7" title="Points and completion" icon={<Target className="h-7 w-7 text-wine" />}><form onSubmit={saveProgrammeRules} className="grid gap-4 md:grid-cols-2"><Field label="Required activities" value={rulesForm.requiredActivities} onChange={value => setRulesForm(current => ({ ...current, requiredActivities: value }))} wide /><label className="grid gap-2 text-sm font-semibold">Minimum programme score<input value={rulesForm.minimumScore} onChange={event => setRulesForm(current => ({ ...current, minimumScore: event.target.value }))} type="number" min="0" max="100" className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-semibold">Minimum examination score<input value={rulesForm.examinationScore} onChange={event => setRulesForm(current => ({ ...current, examinationScore: event.target.value }))} type="number" min="0" max="100" className="border border-wine/20 px-3 py-3 font-normal" /></label><button disabled={saving || !programmeId} className="button-primary w-fit"><Save className="h-4 w-4" />Save completion rules</button></form><Notice>Server validation remains authoritative. Course Studio shows the plan, while learner completion and certificate eligibility continue to be calculated by NIU’s existing protected functions.</Notice></StudioPanel>}
        {step === "certificate" && <StudioPanel eyebrow="Step 8" title="Certificate design" icon={<Target className="h-7 w-7 text-wine" />}><form onSubmit={saveProgrammeRules} className="grid gap-4 md:grid-cols-2"><Field label="Certificate template key" value={certificateForm.templateKey} onChange={value => setCertificateForm(current => ({ ...current, templateKey: value }))} /><Field label="University logo or approved reference" value={programmeForm.image} onChange={value => update(setProgrammeForm, "image", value)} /><Field label="President name" value={certificateForm.presidentName} onChange={value => setCertificateForm(current => ({ ...current, presidentName: value }))} /><Field label="Signature identifier" value={certificateForm.signatureIdentifier} onChange={value => setCertificateForm(current => ({ ...current, signatureIdentifier: value }))} /><div className="border border-gold/40 bg-gold/10 p-6 md:col-span-2"><h4 className="font-serif text-2xl">NIU certificate-only credential</h4><p className="mt-3 text-sm leading-6 text-ink/70">Student name · programme name · credential number · completion date · issue date · learning hours · final score · QR verification</p><p className="mt-3 text-sm font-bold text-wine">President and Founder: Akin S. Sokpah · Signature identifier: akinssokpah</p></div><button disabled={saving || !programmeId} className="button-primary w-fit"><Save className="h-4 w-4" />Save certificate settings</button></form>{certificateTemplateId && <WorkflowAction recordType="certificate_template" recordId={certificateTemplateId} status={certificateTemplateStatus} onTransition={transitionRecord} saving={saving} />}</StudioPanel>}
        {step === "preview" && <StudioPanel eyebrow="Step 9" title="Student preview" icon={<BookOpen className="h-7 w-7 text-wine" />}><p className="text-sm leading-6 text-ink/65">This preview is read-only. It reflects the selected course’s real records and never grants a learner access to draft materials.</p><div className="mt-5 grid gap-3">{modules.filter(item => item.course_id === courseId).map(item => <div key={item.id} className="border border-wine/10 bg-canvas p-4"><b>Module {item.position + 1}: {item.title}</b><p className="mt-1 text-sm text-ink/60">{lessons.filter(lesson => lesson.module_id === item.id).length} lessons · {item.learning_level}</p></div>)}</div></StudioPanel>}
        {step === "transcript" && <StudioPanel eyebrow="Step 10" title="Transcript and completion view" icon={<BookOpen className="h-7 w-7 text-wine" />}><p className="text-sm leading-6 text-ink/65">Review the learner-facing completion evidence that will feed NIU’s protected transcript and certificate workflows. This workspace does not create enrolments, grades, transcripts, or credentials.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><Link href="/transcript" className="border border-wine/10 bg-canvas p-5"><p className="eyebrow">Learner continuity</p><h4 className="mt-2 font-serif text-2xl">Open transcript view</h4><p className="mt-2 text-sm text-ink/60">Inspect the existing protected transcript presentation.</p></Link><Link href="/credentials" className="border border-wine/10 bg-canvas p-5"><p className="eyebrow">Credential continuity</p><h4 className="mt-2 font-serif text-2xl">Open credentials</h4><p className="mt-2 text-sm text-ink/60">Issued credentials remain registrar-controlled and certificate-only.</p></Link></div><Notice>Programme completion remains calculated from enrolled learner activity and released grades. Publication does not issue a certificate automatically.</Notice></StudioPanel>}
        {step === "validate" && <StudioPanel eyebrow="Step 11" title="Validate programme package" icon={<ClipboardCheck className="h-7 w-7 text-wine" />}><div className="mb-5 flex flex-wrap items-center justify-between gap-3 border border-wine/10 bg-canvas p-4"><div><p className="font-semibold">Live database validation</p><p className="text-sm text-ink/60">Counts only records linked to the selected programme package.</p></div><button type="button" onClick={() => void runReadiness()} disabled={saving || !programmeId} className="button-secondary">{saving ? "Validating…" : "Validate programme"}</button></div><div className="grid gap-3">{checks.map(item => <div key={item.label} className={`flex items-center gap-3 border p-4 ${item.ok ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-wine/15 bg-wine/5 text-ink/75"}`}>{item.ok ? <Check className="h-5 w-5 text-emerald-700" /> : <AlertCircle className="h-5 w-5 text-wine" />}<span className="text-sm font-semibold">{item.label}</span><span className="ml-auto text-xs font-bold uppercase">{item.ok ? "Complete" : "Required review"}</span></div>)}</div><div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => setStep("publish")} disabled={!readiness?.ready} className="button-primary">Continue to Publish <ArrowRight className="h-4 w-4" /></button></div><p className="mt-4 text-xs leading-5 text-ink/55">Validation is database-authoritative. Critical missing materials, assessments, or review decisions must be resolved before the controlled release step.</p></StudioPanel>}
        {step === "publish" && <StudioPanel eyebrow="Step 12" title="Publish certificate bundle" icon={<Send className="h-7 w-7 text-wine" />}><div className="border border-gold/40 bg-gold/10 p-5"><p className="font-semibold">Controlled release</p><p className="mt-2 text-sm leading-6 text-ink/65">Publishing is available only after the server-side readiness function passes. It changes the verified programme package through NIU’s protected RPC and records an audit event; it never issues a learner credential automatically.</p></div><div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="border border-wine/10 bg-canvas p-4"><p className="text-3xl font-semibold">{checks.filter(item => item.ok).length}/{checks.length}</p><p className="mt-1 text-sm text-ink/60">validation checks complete</p></div><div className="border border-wine/10 bg-canvas p-4"><p className="text-3xl font-semibold">{selectedProgramme?.status ?? "draft"}</p><p className="mt-1 text-sm text-ink/60">current programme status</p></div></div><div className="mt-6 flex flex-wrap items-center gap-3"><button type="button" onClick={() => void publishProgramme()} disabled={saving || !readiness?.ready || !canPublish} className="button-primary"><Send className="h-4 w-4" />{saving ? "Publishing securely…" : canPublish ? "Publish complete bundle" : "Administrator approval required"}</button><button type="button" onClick={() => setStep("validate")} className="button-secondary">Back to Validate</button></div><p className="mt-4 text-xs leading-5 text-ink/55">Instructor access can build and submit drafts; only administrator roles can perform the final publish action.</p></StudioPanel>}
        {step === "release" && <StudioPanel eyebrow="Step 13" title="Release evidence" icon={<Check className="h-7 w-7 text-wine" />}><p className="text-sm leading-6 text-ink/65">Final release evidence is read-only. Confirm that the package, approval decisions, privacy boundary, certificate-only scope, and public verification path are all documented before closing the work.</p><div className="mt-6 grid gap-3">{["All required records are connected through real UUID relationships", "Private learning resources remain behind enrolled-course authorization", "Programme publication is administrator-controlled and audit-recorded", "Learner transcript and credential views remain available without direct draft access", "No degree, accreditation, or automatic credential claim is introduced"].map(item => <div key={item} className="flex items-center gap-3 border border-wine/10 bg-canvas p-4 text-sm"><Check className="h-5 w-5 text-emerald-700" />{item}</div>)}</div><div className="mt-6 flex flex-wrap gap-3"><Link href="/transcript" className="button-secondary">Open transcript</Link><Link href="/verify" className="button-secondary">Open verification</Link></div></StudioPanel>}
      </main>
...
function ContentStudioPanel({ lessons, items, lessonId, setLessonId, title, setTitle, description, setDescription, file, setFile, onSubmit, saving, noteTitle, setNoteTitle, noteDescription, setNoteDescription, noteBody, setNoteBody, noteRequired, setNoteRequired, noteMinutes, setNoteMinutes, notePoints, setNotePoints, onNoteSubmit, onTransition }: { lessons: Lesson[]; items: ContentItem[]; lessonId: string; setLessonId: (id: string) => void; title: string; setTitle: (value: string) => void; description: string; setDescription: (value: string) => void; file: File | null; setFile: (value: File | null) => void; onSubmit: (event: React.FormEvent) => void; saving: boolean; noteTitle: string; setNoteTitle: (value: string) => void; noteDescription: string; setNoteDescription: (value: string) => void; noteBody: string; setNoteBody: (value: string) => void; noteRequired: boolean; setNoteRequired: (value: boolean) => void; noteMinutes: string; setNoteMinutes: (value: string) => void; notePoints: string; setNotePoints: (value: string) => void; onNoteSubmit: (event: React.FormEvent) => void; onTransition: (recordType: "programme" | "course" | "module" | "lesson" | "content_item" | "assessment" | "certificate_template", recordId: string, targetStatus: "review" | "approved") => Promise<void> }) {
  const plainNoteText = getStructuredText(noteBody).trim();
  const words = plainNoteText ? plainNoteText.split(/\s+/).length : 0;
  const readingMinutes = Math.max(1, Math.ceil(words / 200));
  return <StudioPanel eyebrow="Step 4" title="Learning content" icon={<FileText className="h-7 w-7 text-wine" />}>
    <p className="leading-7 text-ink/65">Write complete academic notes directly in NIU or attach a protected resource. Notes are stored as sanitized structured JSON, kept draft-only until review, and shown to enrolled learners only after publication.</p>
    <form onSubmit={onNoteSubmit} className="mt-6 grid gap-4 border border-gold/40 bg-gold/10 p-5 md:grid-cols-2">
      <div className="md:col-span-2"><p className="eyebrow">Create learning notes</p><h3 className="mt-2 font-serif text-2xl">Write inside NIU</h3><p className="mt-1 text-sm text-ink/65">Use the academic section structure and formatting toolbar. Content stays draft-only until governed review.</p></div>
      <label className="grid gap-2 text-sm font-semibold">Lesson<select required value={lessonId} onChange={event => setLessonId(event.target.value)} className="border border-wine/20 bg-white px-3 py-3 font-normal"><option value="">Choose a lesson</option>{lessons.map(lesson => <option key={lesson.id} value={lesson.id}>{lesson.title}</option>)}</select></label>
      <label className="grid gap-2 text-sm font-semibold">Note title<input required value={noteTitle} onChange={event => setNoteTitle(event.target.value)} className="border border-wine/20 px-3 py-3 font-normal" /></label>
      <label className="grid gap-2 text-sm font-semibold md:col-span-2">Description<textarea value={noteDescription} onChange={event => setNoteDescription(event.target.value)} rows={2} className="border border-wine/20 px-3 py-3 font-normal" /></label>
      <div className="md:col-span-2"><label className="mb-2 block text-sm font-semibold">Academic note body</label><StructuredLessonEditor value={noteBody} onChange={setNoteBody} /></div>
      <div className="flex flex-wrap items-center gap-3 text-xs text-ink/60 md:col-span-2"><span>{words} words</span><span>Estimated reading time: {readingMinutes} min</span><span>Structured JSON is sanitized before storage; unsafe HTML is never executed.</span></div>
      <label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={noteRequired} onChange={event => setNoteRequired(event.target.checked)} />Required learning content</label>
      <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-semibold">Reading minutes<input type="number" min="0" value={noteMinutes} onChange={event => setNoteMinutes(event.target.value)} className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-semibold">Points<input type="number" min="0" value={notePoints} onChange={event => setNotePoints(event.target.value)} className="border border-wine/20 px-3 py-3 font-normal" /></label></div>
      <button disabled={saving} className="button-primary w-fit md:col-span-2"><Save className="h-4 w-4" />{saving ? "Saving note…" : "Save draft learning note"}</button>
    </form>
    {noteBody && <div className="mt-5 border border-wine/10 bg-white p-5"><p className="eyebrow">Editor state</p><p className="mt-3 text-sm leading-6 text-ink/65">{words} words are ready across the structured academic sections. Use Student Preview later to review the read-only learner presentation.</p></div>}
    <form onSubmit={onSubmit} className="mt-7 grid gap-4 border-t border-wine/10 pt-6 md:grid-cols-2"><div className="md:col-span-2"><p className="eyebrow">Protected resources</p><h3 className="mt-2 font-serif text-2xl">Upload a file</h3></div><label className="grid gap-2 text-sm font-semibold">Resource title<input value={title} onChange={event => setTitle(event.target.value)} className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-semibold">Description<textarea value={description} onChange={event => setDescription(event.target.value)} rows={2} className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="grid gap-2 text-sm font-semibold md:col-span-2">Private file<input type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.md,.png,.jpg,.jpeg,.webp,.mp3,.mp4" onChange={event => setFile(event.target.files?.[0] ?? null)} className="border border-wine/20 bg-white px-3 py-3 font-normal" /><span className="text-xs font-normal text-ink/55">Maximum 10 MB. Files remain private and are delivered through enrolled-course authorization.</span></label><button disabled={saving} className="button-secondary w-fit md:col-span-2"><Plus className="h-4 w-4" />{saving ? "Uploading securely…" : "Upload and attach resource"}</button></form>
    <div className="mt-7 border border-wine/10 bg-canvas p-5"><h3 className="font-serif text-2xl">Attached learning content</h3>{items.filter(item => !lessonId || item.lesson_id === lessonId).length ? <div className="mt-4 grid gap-3">{items.filter(item => !lessonId || item.lesson_id === lessonId).map(item => <article key={item.id} className="border border-wine/10 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold">{item.title}</p><p className="text-xs uppercase tracking-[0.1em] text-ink/50">{item.content_format ?? item.category} · {item.status ?? "draft"} · {item.file_name}</p></div><WorkflowAction recordType="content_item" recordId={item.id} status={item.status ?? "draft"} onTransition={onTransition} saving={saving} /></div></article>)}</div> : <p className="mt-3 text-sm text-ink/60">No protected content is attached to this lesson yet.</p>}</div><div className="mt-7 border border-wine/10 bg-canvas p-5"><h3 className="font-serif text-2xl">Step 5 governance</h3><p className="mt-2 text-sm leading-6 text-ink/65">New notes and resources begin as Draft. Use the governed Review → Approved workflow before they can contribute to learner readiness. Existing lesson and programme selections stay unchanged.</p></div>
  </StudioPanel>;
}
function AssessmentStudioPanel({ form, setForm, onSubmit, saving, disabled, assessment, rulesForm, setRulesForm, onSaveRules, onValidate, readiness, onTransition, userId, onMessage, onError }: { form: { title: string; type: string; passingScore: string; attempts: string; timeLimit: string; weight: string }; setForm: React.Dispatch<React.SetStateAction<{ title: string; type: string; passingScore: string; attempts: string; timeLimit: string; weight: string }>>; onSubmit: (event: React.FormEvent) => void; saving: boolean; disabled: boolean; assessment: Assessment | null; rulesForm: { requiredActivities: string; minimumScore: string; examinationScore: string }; setRulesForm: React.Dispatch<React.SetStateAction<{ requiredActivities: string; minimumScore: string; examinationScore: string }>>; onSaveRules: (event: React.FormEvent) => void; onValidate: () => Promise<void>; readiness: Readiness | null; onTransition: (recordType: "assessment", recordId: string, targetStatus: "review" | "approved") => Promise<void>; userId: string | null; onMessage: (message: string) => void; onError: (message: string) => void }) {
  ...
  return <StudioPanel eyebrow="Step 6" title="Assessments" icon={<ClipboardCheck className="h-7 w-7 text-wine" />}>
...
function InlineQuestionStudio({ assessmentId, userId, saving, onMessage, onError }: { assessmentId: string; userId: string | null; saving: boolean; onMessage: (message: string) => void; onError: (message: string) => void }) {
  ...
  async function loadQuestionRecords() {
    const [banksResult, questionsResult] = await Promise.all([
      supabase.from("question_banks").select("id,title,status").order("created_at", { ascending: false }).limit(100),
      supabase.from("questions").select("id,question_bank_id,prompt,question_type,choices,answer_key,points,approval_status").in("approval_status", ["draft", "review", "approved", "published"]).order("created_at", { ascending: false }).limit(100),
    ]);
    if (banksResult.error || questionsResult.error) { onError("Question-bank records could not be loaded inside the Academic Production Studio."); return; }
    setBanks((banksResult.data ?? []) as Array<{ id: string; title: string; status: string }>);
    setQuestions((questionsResult.data ?? []) as InlineQuestion[]);
    setBankId(current => current || banksResult.data?.[0]?.id || "");
  }
  useEffect(() => { if (assessmentId) void loadQuestionRecords(); }, [assessmentId]);

  async function createBank(event: React.FormEvent) {
    event.preventDefault();
    if (!userId || newBankTitle.trim().length < 2) { onError("Provide a question-bank title before saving."); return; }
    setLoading(true);
    const { data, error } = await supabase.from("question_banks").insert({ title: newBankTitle.trim(), status: "draft", governed_workflow: true }).select("id").single();
    if (error || !data) onError(error?.message ?? "The question bank could not be created.");
    else { setNewBankTitle(""); setBankId(data.id); onMessage("Private draft question bank created inside the unified studio."); await loadQuestionRecords(); }
    setLoading(false);
  }
  async function createQuestion(event: React.FormEvent) {
    ...
    if (!bankId || prompt.trim().length < 2) { onError("Choose a question bank and provide a question prompt."); return; }
    if (questionType === "multiple_choice" && cleanedChoices.length < 2) { onError("Multiple-choice questions require at least two answer choices."); return; }
    if (!Number.isFinite(Number(points)) || Number(points) <= 0) { onError("Question points must be greater than zero."); return; }
    setLoading(true);
    const selectedCorrect = ["multiple_choice", "true_false"].includes(questionType) ? (cleanedChoices.some(choice => choice.id === correctChoiceId) ? correctChoiceId : cleanedChoices[0]?.id) : undefined;
    const { error } = await supabase.from("questions").insert({ question_bank_id: bankId, question_type: questionType, prompt: prompt.trim(), choices: cleanedChoices, answer_key: selectedCorrect ? { correct_choice_id: selectedCorrect } : {}, points: Number(points), approval_status: "draft", governed_workflow: true });
    if (error) onError(error.message);
    else { setPrompt(""); setChoices([{ id: "1", text: "" }, { id: "2", text: "" }]); setCorrectChoiceId("1"); setPoints("1"); onMessage("Question saved as a private draft. Submit it for governed review before attaching it to an assessment."); await loadQuestionRecords(); }
    setLoading(false);
  }
  async function transitionQuestion(questionId: string, targetStatus: "review" | "approved") {
    setLoading(true);
    const { error } = await supabase.rpc("niu_transition_academic_record", { target_type: "question", target_id: questionId, target_status: targetStatus });
    if (error) onError(`Question could not move to ${targetStatus}: ${error.message}`);
    else { onMessage(`Question moved to ${targetStatus}.`); await loadQuestionRecords(); }
    setLoading(false);
  }
  async function attachQuestion(questionId: string) {
    if (!assessmentId) { onError("Save an assessment before attaching approved questions."); return; }
    setLoading(true);
    const { data: positionRows } = await supabase.from("assessment_questions").select("position").eq("assessment_id", assessmentId).order("position", { ascending: false }).limit(1);
    const { error } = await supabase.from("assessment_questions").upsert({ assessment_id: assessmentId, question_id: questionId, position: Number(positionRows?.[0]?.position ?? -1) + 1 }, { onConflict: "assessment_id,question_id" });
    if (error) onError(`Approved question could not be attached: ${error.message}`); else { onMessage("Approved question attached to the assessment sequence."); await loadQuestionRecords(); }
    setLoading(false);
  }
  return <section className="mt-7 border-t border-wine/10 pt-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="eyebrow">Embedded question authoring</p><h4 className="mt-2 font-serif text-2xl">Question bank and approved attachments</h4><p className="mt-2 text-sm leading-6 text-ink/65">Create private banks and questions here. Only questions that have completed Draft → Review → Approved can be attached to the assessment.</p></div><span className="border border-gold/40 bg-gold/10 px-3 py-2 text-xs font-bold uppercase tracking-[0.12em] text-wine">No route change</span></div><div className="mt-5 grid gap-5 lg:grid-cols-2"><form onSubmit={createBank} className="border border-wine/10 bg-canvas p-5"><h5 className="font-serif text-xl">Create question bank</h5><input value={newBankTitle} onChange={event => setNewBankTitle(event.target.value)} placeholder="e.g. Digital literacy checks" className="mt-4 w-full border border-wine/20 bg-white px-3 py-3 text-sm" /><button disabled={loading || saving} className="button-secondary mt-3">Create private bank</button></form><form onSubmit={createQuestion} className="border border-wine/10 bg-canvas p-5"><h5 className="font-serif text-xl">Create draft question</h5><select value={bankId} onChange={event => setBankId(event.target.value)} className="mt-4 w-full border border-wine/20 bg-white px-3 py-3 text-sm"><option value="">Choose a bank</option>{banks.map(bank => <option key={bank.id} value={bank.id}>{bank.title} · {bank.status}</option>)}</select><textarea value={prompt} onChange={event => setPrompt(event.target.value)} rows={3} placeholder="Question prompt" className="mt-3 w-full border border-wine/20 bg-white px-3 py-3 text-sm" /><div className="mt-3 grid gap-2">{choices.map((choice, index) => <input key={choice.id} value={choice.text} onChange={event => setChoices(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, text: event.target.value } : item))} placeholder={`Choice ${index + 1}`} className="border border-wine/20 bg-white px-3 py-2 text-sm" />)}</div><div className="mt-3 flex flex-wrap gap-2"><label className="text-xs font-bold uppercase tracking-[0.1em] text-ink/55">Correct<select value={correctChoiceId} onChange={event => setCorrectChoiceId(event.target.value)} className="ml-2 border border-wine/20 bg-white px-2 py-2 text-xs normal-case tracking-normal"><option value="1">Choice 1</option><option value="2">Choice 2</option></select></label><label className="text-xs font-bold uppercase tracking-[0.1em] text-ink/55">Points<input type="number" min="1" value={points} onChange={event => setPoints(event.target.value)} className="ml-2 w-16 border border-wine/20 bg-white px-2 py-2 text-xs normal-case tracking-normal" /></label></div><button disabled={loading || saving || !bankId} className="button-secondary mt-3">Save draft question</button></form></div><div className="mt-5 border border-wine/10 bg-white p-5"><h5 className="font-serif text-xl">Approved reusable questions</h5>{questions.filter(question => !bankId || question.question_bank_id === bankId).length ? <div className="mt-3 grid gap-2">{questions.filter(question => !bankId || question.question_bank_id === bankId).map(question => <div key={question.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-wine/10 py-3"><div><p className="text-sm text-ink/75">{question.prompt}</p><p className="mt-1 text-xs uppercase tracking-[0.1em] text-ink/45">{question.question_type.replace("_", " ")} · {question.points} points · {question.approval_status}</p></div><div className="flex flex-wrap gap-2">{question.approval_status === "draft" && <button type="button" disabled={loading || saving} onClick={() => void transitionQuestion(question.id, "review")} className="button-secondary px-3 py-2 text-xs">Submit for review</button>}{question.approval_status === "review" && <button type="button" disabled={loading || saving} onClick={() => void transitionQuestion(question.id, "approved")} className="button-secondary px-3 py-2 text-xs">Approve question</button>}{["approved", "published"].includes(question.approval_status) && <button type="button" disabled={loading || saving || !assessmentId} onClick={() => void attachQuestion(question.id)} className="button-secondary px-3 py-2 text-xs">Attach approved question</button>}</div></div>)}</div> : <p className="mt-3 text-sm text-ink/60">No approved reusable questions are available for this bank yet.</p>}</div></section>;
}
