...
type Assessment = { id: string; title: string; status: string; course_id: string };
...
export default function CourseStudio() {
  ...
  async function loadStructure(nextProgrammeId?: string) {
    ...
    const { data: assessmentData, error: assessmentError } = await supabase.from("assessments").select("id,title,status,course_id").in("course_id", ids).order("created_at");
  ...
  async function createAssessment(event: React.FormEvent) {
    ...
    else { setAssessmentId(createdAssessment.id); setNotice("Draft assessment saved. Submit it for review before approval."); setAssessmentForm({ title: "", type: "knowledge_check", passingScore: "70", attempts: "1", timeLimit: "", weight: "0" }); await loadStructure(programmeId); }
    setSaving(false);
  }
  ...
  return <SiteShell>
    ...
    <section className="mx-auto max-w-[1500px] px-4 py-6 sm:px-8"><div className="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)_300px]">
      ...
      <main className="min-w-0">
        ...
        {step === "assessment" && <AssessmentStudioPanel form={assessmentForm} setForm={setAssessmentForm} onSubmit={createAssessment} saving={saving} disabled={!courseId} />}{assessments.find(item => item.id === assessmentId) && step === "assessment" && <WorkflowAction recordType="assessment" recordId={assessmentId} status={assessments.find(item => item.id === assessmentId)?.status ?? "draft"} onTransition={transitionRecord} saving={saving} />}
