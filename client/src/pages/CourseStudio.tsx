...
export default function CourseStudio() {
  ...
  async function createInlineNote(event: React.FormEvent) {
    event.preventDefault(); clearFeedback();
    const targetLessonId = lessonId || selectedLessons[0]?.id;
    if (!userId || !targetLessonId || noteTitle.trim().length < 3 || noteBody.trim().length < 40) { setError("Choose a lesson, provide a note title, and write at least 40 characters of meaningful learning content."); return; }
    const targetLesson = lessons.find(item => item.id === targetLessonId);
    ...
    const { data: item, error: itemError } = await supabase.from("content_library_items").insert({ id: noteId, title: noteTitle.trim(), category: "study_guide", file_name: `${slugify(noteTitle)}.md`, content_type: "text/markdown", storage_path: `inline-notes/${noteId}.md`, description: noteDescription.trim() || null, inline_content: noteBody.trim(), content_format: "markdown", estimated_minutes: Number(noteMinutes) || 0, points: Number(notePoints) || 0, status: "draft", governed_workflow: true, created_by: userId }).select("id").single();
...
function ContentStudioPanel({ lessons, lessonId, setLessonId, title, setTitle, description, setDescription, file, setFile, onSubmit, saving, noteTitle, setNoteTitle, noteDescription, setNoteDescription, noteBody, setNoteBody, noteRequired, setNoteRequired, noteMinutes, setNoteMinutes, notePoints, setNotePoints, onNoteSubmit }: { lessons: Lesson[]; lessonId: string; setLessonId: (id: string) => void; title: string; setTitle: (value: string) => void; description: string; setDescription: (value: string) => void; file: File | null; setFile: (value: File | null) => void; onSubmit: (event: React.FormEvent) => void; saving: boolean; noteTitle: string; setNoteTitle: (value: string) => void; noteDescription: string; setNoteDescription: (value: string) => void; noteBody: string; setNoteBody: (value: string) => void; noteRequired: boolean; setNoteRequired: (value: boolean) => void; noteMinutes: string; setNoteMinutes: (value: string) => void; notePoints: string; setNotePoints: (value: string) => void; onNoteSubmit: (event: React.FormEvent) => void }) {
  const words = noteBody.trim() ? noteBody.trim().split(/\s+/).length : 0;
  const readingMinutes = Math.max(1, Math.ceil(words / 200));
  return <StudioPanel eyebrow="Step 4" title="Learning content" icon={<FileText className="h-7 w-7 text-wine" />}>
    <p className="leading-7 text-ink/65">Write complete academic notes directly in NIU or attach a protected resource. Notes are stored as Markdown, kept draft-only until review, and shown to enrolled learners only after publication.</p>
    <form onSubmit={onNoteSubmit} className="mt-6 grid gap-4 border border-gold/40 bg-gold/10 p-5 md:grid-cols-2">
      <div className="md:col-span-2"><p className="eyebrow">Create learning notes</p><h3 className="mt-2 font-serif text-2xl">Write inside NIU</h3><p className="mt-1 text-sm text-ink/65">Use simple Markdown headings, lists, links, blockquotes, and code blocks without uploading a textbook.</p></div>
      ...
      <label className="grid gap-2 text-sm font-semibold md:col-span-2">Academic note body<textarea required value={noteBody} onChange={event => setNoteBody(event.target.value)} rows={14} placeholder="# Introduction\n\nWrite the lesson here...\n\n## Key concepts\n- Concept one" className="border border-wine/20 px-3 py-3 font-mono text-sm font-normal" /></label>
      <div className="flex flex-wrap items-center gap-3 text-xs text-ink/60 md:col-span-2"><span>{words} words</span><span>Estimated reading time: {readingMinutes} min</span><span>Markdown is stored as text, so unsafe HTML is not executed.</span></div>
    ...
    {noteBody && <div className="mt-5 border border-wine/10 bg-white p-5"><p className="eyebrow">Preview</p><div className="mt-3 whitespace-pre-wrap leading-7 text-ink/80">{noteBody}</div></div>}
