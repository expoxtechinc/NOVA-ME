...
export default function AssessmentBuilder() {
  ...
  async function updateQuestionStatus(question: Question, status: Status) {
    if ((status === "approved" || status === "published" || status === "archived") && !canApprove) { setError("Only an administrator, registrar, or Super Administrator may approve, publish, or archive questions."); return; }
    setSaving(true); setError(null); setMessage(null);
  ...
  async function updateAssessmentStatus(assessment: Assessment, status: Assessment["status"]) {
    ...
    setSaving(true); setError(null); setMessage(null);
    const { error: writeError } = await supabase.from("assessments").update({ status }).eq("id", assessment.id);
