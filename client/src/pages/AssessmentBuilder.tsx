...
type Status = "draft" | "review" | "approved" | "rejected" | "archived";
...
export default function AssessmentBuilder() {
  ...
  async function saveBank(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    const { error: writeError } = await supabase.from("question_banks").insert({ title: newBankTitle.trim(), description: newBankDescription.trim() || null, status: "draft" });
  ...
  async function saveQuestion(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { question_bank_id: bankId, question_type: questionType, prompt: prompt.trim(), choices: cleanedChoices, answer_key: selectedCorrect ? { correct_choice_id: selectedCorrect } : {}, explanation: explanation.trim() || null, difficulty, topic: topic.trim() || null, learning_objective: learningObjective.trim() || null, points: Number(points), requires_manual_grading: ["essay", "short_answer", "scenario"].includes(questionType) };
  ...
  async function updateQuestionStatus(question: Question, status: Status) {
    ...
    setSaving(true); setError(null); setMessage(null);
    const { error: writeError } = await supabase.from("questions").update({ approval_status: status }).eq("id", question.id);
    if (writeError) setError(writeError.message); else { setMessage(`Question is now ${status}.`); await load(); }
    setSaving(false);
  }
  ...
  async function attachQuestion(question: Question) {
    ...
    if (writeError) setError(writeError.message); else setMessage("Approved question attached to the selected assessment sequence.");
    setSaving(false);
  }
