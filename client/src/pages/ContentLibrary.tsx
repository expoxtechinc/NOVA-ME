...
> Original NIU learning material for enrolled learners.

## Purpose

This original NIU study guide supports the first module of **Digital Foundations for Enterprise and Remote Work**. It helps learners establish inclusive, organised, and responsible study habits before using collaboration tools or planning a project. It is a learning resource for a certificate-only programme. It is not professional advice, a licence, or a guarantee of employment.
...
This is original NIU teaching material written for this course structure. It does not reproduce third-party course content.
`;
...
export default function ContentLibrary() {
  ...
  async function upload(event: React.FormEvent) {
    ...
    if (!title.trim() || !selectedFile) {
      setError("Provide a resource title and choose a file.");
      return;
    }
    if (selectedFile.size > maxBytes) {
      setError("Learning resources must be 10 MB or smaller.");
      return;
    }
    if (!categoryMimeTypes[category].includes(selectedFile.type)) {
      setError("This file type is not supported for the selected content category.");
      return;
    }
    setSaving(true);
    try {
      ...
      setError(errorMessage(caught as { message?: string }, "NIU could not upload this learning resource."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function initializeDigitalStudyGuide() {
    ...
    try {
      ...
      setError(errorMessage(caught as { message?: string }, "NIU could not add the protected study guide."));
    } finally {
      setSaving(false);
    }
  }

  async function repairDigitalStudyGuideEncoding() {
    if (!window.confirm("Replace the private first Digital Foundations guide with corrected learner-facing text? The learning-material record, lesson attachment, published course, enrollments, and access rules will remain unchanged.")) return;
    setSaving(true);
    setMessage(null);
    setError(null);
    try {
      const { data: sessionData } = await supabase.auth.getSession();
      const user = sessionData.session?.user;
      if (!user) throw new Error("Sign in to NIU before repairing the protected study guide.");
      const { data: existing, error: existingError } = await supabase.from("content_library_items").select("id").eq("title", starterGuideTitle).eq("file_name", starterGuideFilename).maybeSingle();
      if (existingError || !existing) throw new Error(existingError?.message ?? "NIU could not find the original study guide record.");
      const replacementPath = storagePath(user.id, starterGuideFilename);
      const repairedGuideFile = new File([starterGuide], starterGuideFilename, { type: "text/markdown" });
      const { error: storageError } = await supabase.storage.from(storageBucket).upload(replacementPath, repairedGuideFile, { contentType: "text/markdown", upsert: false });
      if (storageError) throw new Error(storageError.message);
      const { error: recordError } = await supabase.from("content_library_items").update({ storage_path: replacementPath }).eq("id", existing.id);
      if (recordError) throw new Error(recordError.message);
      setMessage("The first private Digital Foundations study guide now uses corrected learner-facing text. Its record, attachment, and enrolled-only access control remain unchanged.");
      await load();
    } catch (caught) {
      setError(errorMessage(caught as { message?: string }, "NIU could not repair the protected study guide text."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function addExternal(event: React.FormEvent) {
    ...
    try {
      ...
      setError("Enter a complete https:// external resource URL.");
      return;
    }
    if (url.protocol !== "https:" || title.trim().length < 3) {
      setError("Provide a title and an https:// external resource URL.");
      return;
    }
    setSaving(true);
    const { data: sessionData } = await supabase.auth.getSession();
    ...
    if (insertError) setError(insertError.message);
      ...
      setMessage("External learning resource added to the protected NIU library record.");
      setTitle("");
      setDescription("");
      setExternalUrl("");
      await load();
  ...
  async function attach(event: React.FormEvent) {
    ...
    setMessage(null);
    setError(null);
    if (!attachLessonId || !attachItemId) {
      setError("Choose a lesson and a content-library item to attach.");
      return;
    }
    setSaving(true);
    try {
      await attachItem(attachLessonId, attachItemId);
      setMessage("Learning resource attached to the selected lesson.");
      setAttachItemId("");
    } catch (caught) {
      setError(errorMessage(caught as { message?: string }, "NIU could not attach this learning resource."));
    } finally {
      setSaving(false);
    }
  }
  ...
  if (!staff) {
    return <SiteShell><section className="mx-auto flex min-h-[55vh] max-w-2xl flex-col items-center justify-center px-5 text-center"><ShieldAlert className="h-10 w-10 text-wine" /><h1 className="mt-5 font-serif text-4xl">Academic staff authority required.</h1><p className="mt-3 leading-7 text-ink/65">NIU only allows protected content-library administration for authorised institutional roles.</p><Link href={role ? "/portal" : "/signin"} className="button-primary mt-7">{role ? "Return to My NIU" : "Sign in to NIU"}</Link></section></SiteShell>;
  }
    ...
    <SiteShell>
      <section className="border-b border-wine/10 bg-canvas">
        <div className="mx-auto max-w-[1200px] px-5 py-12 sm:px-8">
          ...
          <h1 className="mt-4 font-serif text-5xl tracking-[-0.04em]">Govern reusable learning materials.</h1>
          <p className="mt-3 max-w-3xl leading-7 text-ink/65">Upload supported teaching resources once, attach them to authorised lessons, and preserve enrolled-only delivery. Files remain private in NIU object storage; external resources are explicitly labelled and require an https address.</p>
        </div>
      </section>
      <section className="mx-auto grid max-w-[1200px] gap-7 px-5 py-12 sm:px-8 lg:grid-cols-2">
        {(message || error) && <p className={`lg:col-span-2 flex gap-2 border-l-4 p-4 text-sm ${error ? "border-wine bg-wine/5" : "border-emerald-700 bg-emerald-50 text-emerald-900"}`}><AlertCircle className="h-5 w-5 shrink-0" />{error ?? message}</p>}

        {role === "super_admin" && <section className="lg:col-span-2 border border-gold/40 bg-gold/10 p-6"><p className="text-sm font-bold text-wine">NIU first certificate programme</p><h2 className="mt-2 font-serif text-3xl">Prepare and publish the complete first course</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-ink/75">Create four original private study guides, attach one to every required Digital Foundations lesson, then pass NIU’s certificate-only readiness and publication controls.</p><Link href="/first-certificate-release" className="button-primary mt-5 inline-flex">Open controlled certificate release</Link></section>}

        {role === "super_admin" && <section className="lg:col-span-2 border border-gold/40 bg-gold/10 p-6"><p className="text-sm font-bold text-wine">NIU original starter guide</p><h2 className="mt-2 font-serif text-3xl">Maintain the first protected study guide</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-ink/75">The original guide stays private and attached to the first lesson. The text-repair action replaces only its private file bytes; it does not change the material record, lesson attachment, programme publication, learner enrollment, or enrolled-only access control.</p><div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={initializeDigitalStudyGuide} disabled={saving} className="button-secondary">{saving ? "Working…" : "Add NIU draft study guide"}</button><button type="button" onClick={repairDigitalStudyGuideEncoding} disabled={saving} className="button-secondary">{saving ? "Repairing study guide…" : "Repair first study guide text"}</button></div></section>}

        <form onSubmit={upload} className="border border-wine/10 bg-white p-7 shadow-[0_16px_36px_rgba(29,25,21,0.05)]"><FileUp className="h-6 w-6 text-wine" /><h2 className="mt-4 font-serif text-3xl">Upload a learning resource</h2><p className="mt-2 text-sm leading-6 text-ink/60">PDF, DOC/DOCX, PPT/PPTX, approved images, audio, video, text, Markdown, research material, and study guides are categorised before storage. Maximum file size: 10 MB.</p><label className="mt-5 grid gap-2 text-sm font-semibold">Title<input value={title} onChange={event => setTitle(event.target.value)} className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="mt-4 grid gap-2 text-sm font-semibold">Category<select value={category} onChange={event => setCategory(event.target.value as UploadCategory)} className="border border-wine/20 bg-white px-3 py-3 font-normal">{uploadCategories.map(value => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></label><label className="mt-4 grid gap-2 text-sm font-semibold">Description<span className="sr-only">Description</span><textarea value={description} onChange={event => setDescription(event.target.value)} rows={3} className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="mt-4 grid gap-2 text-sm font-semibold">File<input type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.md,.jpg,.jpeg,.png,.webp,.mp3,.wav,.ogg,.mp4,.webm" onChange={event => setSelectedFile(event.target.files?.[0] ?? null)} className="border border-wine/20 bg-white px-3 py-3 font-normal" /></label><button disabled={saving} className="button-primary mt-6">{saving ? "Uploading…" : "Add to protected library"}</button></form>

        <div className="grid gap-7"><form onSubmit={addExternal} className="border border-wine/10 bg-white p-7"><Link2 className="h-6 w-6 text-wine" /><h2 className="mt-4 font-serif text-3xl">Register an external resource</h2><p className="mt-2 text-sm leading-6 text-ink/60">Use only links you are authorised to share. NIU records the reference; it does not claim ownership of external content.</p><label className="mt-5 grid gap-2 text-sm font-semibold">Title<input value={title} onChange={event => setTitle(event.target.value)} className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="mt-4 grid gap-2 text-sm font-semibold">HTTPS URL<input type="url" value={externalUrl} onChange={event => setExternalUrl(event.target.value)} placeholder="https://" className="border border-wine/20 px-3 py-3 font-normal" /></label><label className="mt-4 grid gap-2 text-sm font-semibold">Description<textarea value={description} onChange={event => setDescription(event.target.value)} rows={2} className="border border-wine/20 px-3 py-3 font-normal" /></label><button disabled={saving} className="button-secondary mt-5">Save external resource</button></form><form onSubmit={attach} className="border border-wine/10 bg-canvas p-6"><Paperclip className="h-6 w-6 text-wine" /><h2 className="mt-4 font-serif text-3xl">Attach to a lesson</h2><label className="mt-5 grid gap-2 text-sm font-semibold">Lesson<select value={attachLessonId} onChange={event => setAttachLessonId(event.target.value)} className="border border-wine/20 bg-white px-3 py-3 font-normal"><option value="">Choose a lesson</option>{lessons.map(lesson => <option key={lesson.id} value={lesson.id}>{lesson.title}</option>)}</select></label><label className="mt-4 grid gap-2 text-sm font-semibold">Library item<select value={attachItemId} onChange={event => setAttachItemId(event.target.value)} className="border border-wine/20 bg-white px-3 py-3 font-normal"><option value="">Choose a resource</option>{items.map(item => <option key={item.id} value={item.id}>{item.title} · {item.category}</option>)}</select></label><button disabled={saving} className="button-primary mt-5">{saving ? "Attaching…" : "Attach resource"}</button></form></div>

        <section className="lg:col-span-2 border border-wine/10 bg-white p-7"><div className="flex items-end justify-between gap-4"><div><p className="eyebrow">NIU library records</p><h2 className="mt-2 font-serif text-3xl">Approved content inventory</h2></div><p className="text-sm text-ink/55">{items.length} item{items.length === 1 ? "" : "s"}</p></div>{items.length ? <div className="mt-6 divide-y divide-wine/10">{items.map(item => <article key={item.id} className="flex flex-col justify-between gap-3 py-4 sm:flex-row sm:items-start"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-wine">{item.category.replaceAll("_", " ")}</p><h3 className="mt-1 font-serif text-xl">{item.title}</h3><p className="mt-1 text-sm text-ink/60">{item.file_name}{item.description ? ` · ${item.description}` : ""}</p></div><p className="text-xs text-ink/50">{new Date(item.created_at).toLocaleDateString()}</p></article>)}</div> : <p className="mt-6 text-sm leading-6 text-ink/65">No content-library records yet. Add an approved learning resource when you are ready to attach it to a course lesson.</p>}</section>
      </section>
    </SiteShell>
  );
}
