...
The first real student-enrolment attempt correctly reached NIU’s protected database function but exposed an old type mismatch: its status literals were treated as text while the live `enrollments.status` column uses the `enrollment_status` enum. The function now casts the `active` and `completed` values explicitly to the live enum. It still requires an authenticated active NIU account, a published course, completed prerequisites where applicable, and authenticated-only execution. No enrolment was created by the failed attempt; the real student retry and enrolled-only material-access check remain pending.
