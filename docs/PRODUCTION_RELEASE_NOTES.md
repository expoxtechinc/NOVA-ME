...
The authorised student retry has since produced one active student enrolment for the published first course. The learner’s protected course page displayed the first attached private study guide and the server-verified lesson-completion state. The existing resource action generated no visible signed-link error; however, it used a browser popup, so the exercise did not conclusively show whether the Android browser displayed or downloaded the file. NIU now uses direct same-tab navigation after the signed URL is created and shows an **Opening protected resource…** status first. This prevents popup blocking from obscuring the result. A final real enrolled-learner retrieval on the updated live release remains required before the protected-retrieval validation is closed.
