...
## Master restructure audit — 1 September 2026

The automated audit confirms that the canonical Academic Production Studio, legacy compatibility routes, structured lesson editor, question-bank workflow, curriculum-import workflow, draft-first governance, certificate controls, private-material model, and mobile public shell remain present in the current release. The production repository is synchronized to NOVA-ME, the linked Vercel project is rebuilding from the current main branch, and no programme publication or academic-record mutation was performed during this pass.

The live custom domain was checked from an Android-compatible public path. The homepage renders the NIU application and contact controls; `/api/healthz` returns JSON with the current deployment SHA; the actual `aiBuilder.createPlan` mutation route returns a JSON 405 for an unauthorised GET; and an unknown tRPC procedure returns a JSON 404. The previous function-bootstrap failure is therefore resolved. Full Vitest, TypeScript, production build, and local Vercel-boundary verification pass.

The remaining launch-evidence items are deliberately not represented as completed: a real student email-link return, an owner-authorised People Governance walkthrough, an authenticated blueprint save/research-review run, and a Super Administrator protected upload followed by learner retrieval require a real inbox or authenticated owner session. The browser session available to this audit is unauthenticated, so claiming those observations would be misleading.
