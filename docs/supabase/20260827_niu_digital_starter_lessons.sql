...
begin
  ...
  for v_module in select id, position from public.course_modules where course_id = v_course_id order by position loop
    if v_module.position = 0 then
      ...
      values (v_module.id, 'Access and information check-in', 'article', 'A draft lesson placeholder for approved teaching materials about inclusive access, credible information, and safe digital learning habits.', 0, null, true, '["Identify practical access and account habits for online learning.","Recognise credible information sources for a learning task."]'::jsonb, 60, 0, 'draft');
    elsif v_module.position = 1 then
      insert into public.lessons (module_id, title, kind, description, position, rich_text, is_required, learning_objectives, estimated_minutes, points, status)
      values (v_module.id, 'Collaboration workspace plan', 'article', 'A draft lesson placeholder for approved instructional materials on respectful digital communication, shared tasks, and remote-work privacy. Add an assessed activity later through NIU’s authorised assessment workflow when appropriate.', 0, null, true, '["Prepare a simple plan for respectful and organised remote collaboration.","Identify a privacy or security practice relevant to shared work."]'::jsonb, 90, 0, 'draft');
    elsif v_module.position = 2 then
      insert into public.lessons (module_id, title, kind, description, position, rich_text, is_required, learning_objectives, estimated_minutes, points, status)
      values (v_module.id, 'Opportunity discovery worksheet', 'article', 'A draft lesson placeholder for approved instructional materials exploring a community, audience, or digital-work opportunity. Add an assessed activity later through NIU’s authorised assessment workflow when appropriate.', 0, null, true, '["Describe a relevant problem or opportunity.","Develop a simple evidence-based statement of value and next steps."]'::jsonb, 90, 0, 'draft');
    elsif v_module.position = 3 then
      insert into public.lessons (module_id, title, kind, description, position, rich_text, is_required, learning_objectives, estimated_minutes, points, status)
      values (v_module.id, 'Responsible project-plan reflection', 'article', 'A draft lesson placeholder for approved project-planning and reflection materials that consider ethics, inclusion, privacy, and accessibility. Add an assessed activity later through NIU’s authorised assessment workflow when appropriate.', 0, null, true, '["Prepare a small digital or remote-work project plan.","Reflect on ethical, inclusive, private, and accessible implementation choices."]'::jsonb, 90, 0, 'draft');
    else
      continue;
    end if;
