-- Extend NIU's existing programme readiness gate for future governed assessments only.
...
begin
  ...
  select count(*) into governed_assessment_total from public.program_courses pc join public.assessments a on a.course_id = pc.course_id where pc.program_id = target_program_id and a.governed_workflow;
  select count(*) into ready_governed_assessment_total from public.program_courses pc join public.assessments a on a.course_id = pc.course_id where pc.program_id = target_program_id and a.governed_workflow and a.status in ('approved', 'published') and a.passing_score > 0 and a.passing_score <= 100 and a.time_limit_minutes is not null and a.time_limit_minutes > 0 and a.attempt_limit is not null and a.attempt_limit > 0 and jsonb_typeof(coalesce(a.required_completion_rules, '{}'::jsonb)) = 'object' and exists (select 1 from jsonb_each(coalesce(a.required_completion_rules, '{}'::jsonb)) as rule(key, value)) and exists (select 1 from public.assessment_questions aq join public.questions q on q.id = aq.question_id where aq.assessment_id = a.id and q.approval_status in ('approved', 'published')) and not exists (select 1 from public.assessment_questions aq join public.questions q on q.id = aq.question_id where aq.assessment_id = a.id and q.approval_status not in ('approved', 'published'));
  return jsonb_build_object('program_id', program_record.id, 'program_status', program_record.status, 'courses', course_total, 'approved_courses', approved_course_total, 'modules', module_total, 'approved_modules', approved_module_total, 'required_lessons', required_lesson_total, 'approved_required_lessons', approved_required_lesson_total, 'required_lessons_with_material', required_material_total, 'governed_assessments', governed_assessment_total, 'ready_governed_assessments', ready_governed_assessment_total, 'ready', course_total > 0 and course_total = approved_course_total and module_total > 0 and module_total = approved_module_total and required_lesson_total > 0 and required_lesson_total = approved_required_lesson_total and required_lesson_total = required_material_total and (governed_assessment_total = 0 or governed_assessment_total = ready_governed_assessment_total));
end;
$$;
