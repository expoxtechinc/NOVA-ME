-- Use the live NIU active-account function and an exact core-role check.
-- `niu_is_active_super_admin` is not part of the deployed schema.
...
begin
  if auth.uid() is null
    or not public.niu_account_is_active()
    or not exists (select 1 from public.profiles where id = auth.uid() and role = 'super_admin') then
    raise exception 'Active Super Administrator authority is required';
  end if;

  select p.id, c.id into v_program_id, v_course_id
  ...
  where p.code = 'DSERW-CERT' and p.award_type = 'certificate' and c.slug = 'digital-foundations-enterprise-remote-work';
  if v_program_id is null or v_course_id is null then raise exception 'The authorised NIU starter certificate structure is not available'; end if;
  ...
  if v_module_count <> 4 or v_lesson_count <> 4 or v_material_count <> 4 then raise exception 'The first NIU certificate bundle requires four modules, four required lessons, and protected material attached to every required lesson before approval'; end if;
  ...
  insert into public.audit_events (actor_id, action, subject_type, subject_id, metadata) values (auth.uid(), 'digital_starter_bundle_approved', 'certificate_program', v_program_id, jsonb_build_object('course_id', v_course_id, 'module_count', v_module_count, 'required_lesson_count', v_lesson_count, 'required_lessons_with_material', v_material_count, 'award_type', 'certificate'));
