...
5. Select **Deploy**. Vercel will run `pnpm install --frozen-lockfile` followed by `pnpm build`, serve the Vite build from `dist/public`, and route only API, OAuth, and managed-storage requests through `api/index.ts`.
