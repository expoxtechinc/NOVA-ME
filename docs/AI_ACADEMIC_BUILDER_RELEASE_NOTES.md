...
## Complete draft-package generation correction — 2026-08-28

The AI Academic Builder now continues beyond planning. After administrator research review and evidence-bound generation planning, **Generate Complete Academic Package** compiles the reviewed blueprint into linked private draft records through an authorised transactional Supabase RPC. The package includes a draft school and department relationship, certificate programme, courses, course version snapshots, ordered modules, lessons, protected Markdown study-guide metadata, activities and accessibility metadata, question banks, private questions, assessments, and assessment-question mappings. Generated material is deliberately labelled as a draft and requires administrator authoring and verification; answer keys remain pending review rather than being presented as factual or approved content.

The generator is idempotency-protected per AI Builder job, requires staff authorization, preserves source and missing-evidence boundaries, stores all generated records in draft status, and never approves or publishes content. A read-only quality gate checks real relationships, draft statuses, protected material links, assessment mappings, and the closed publication boundary. A learner-safe preview exposes only course/module/lesson structure and excludes answer keys and administrator controls. Existing NIU publication gates remain the only route to manual approval and publication.

The applied draft-package RPC now persists explicit metadata for video scripts, transcripts, diagrams, references, assignments, and rubrics. Where the blueprint does not provide authoritative material, the generated record stores a visible `Missing:` marker rather than fabricated content; administrators must author, verify, and approve these sections before publication.

## Validation boundary — 2026-08-28

The complete automated validation suite passes with 35 test files and 85 tests, strict TypeScript, production build, production dependency audit, and diff checks. A bounded read-only query of the connected Online University project returned no AI Builder jobs, confirming that validation did not create test academic records. The production sign-in page loads at `https://nova-me-six.vercel.app/signin`, while the available browser session remained unauthenticated. Accordingly, NIU does not claim completion of the real Digital Marketing database run, learner retrieval, or owner-session checks until inspectable authenticated evidence is available.
